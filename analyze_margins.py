...
Computes and visualizes margin distributions to show how embedding quality
...
Usage:
    python analyze_margins.py \
        --data_root CAPTCHA_Models/CAPTCHA_Dataset/images \
        --classes_file CAPTCHA_Models/CAPTCHA_Dataset/classes.txt \
        --models CAPTCHA_Models/Transformers/Fully_Tuned

Settings:
- Evaluation parameters (n_way, k_shot, q_query, n_episodes) are auto-loaded
  from the model's few_shot_results.json config
- Falls back to defaults (3-way 5-shot, 15 queries, 500 episodes) if not found
...
def load_model_variant(architecture: str, variant: str, device: str, model_family: str = None) -> nn.Module:
    """
    ...
    Args:
        ...
        model_family: 'CAPTCHA_Models' or 'Bird_Models' (prepended to paths)

    Returns:
        Embedding network (extracted from PrototypicalNetwork)
    """
    config = MODELS_CONFIG[architecture][variant]
    # Prepend model_family if provided
    if model_family:
        model_path = Path(model_family) / config['path']
        checkpoint_path = Path(model_family) / config['checkpoint']
    else:
...
def analyze_model_margins(
    ...
    Args:
        model_family: 'CAPTCHA_Models' or 'Bird_Models' (prepended to model paths)
    ...
    # Load model
    model = load_model_variant(architecture, variant, device, model_family)
...
def analyze_all_margins(
    ...
    model_family = None  # Will be detected from user input
    if selected_models is None:
        ...
        # Parse user-specified models (e.g., 'CNN/Fully_Tuned' or 'CAPTCHA_Models/CNN/Fully_Tuned')
        models_to_run = []
        for spec in selected_models:
            parts = spec.split('/')
            if len(parts) == 3:
                # Full path: e.g., 'CAPTCHA_Models/Transformers/Fully_Tuned'
                detected_family, arch, var = parts
                if model_family is None:
                    model_family = detected_family
                elif model_family != detected_family:
                    print(f"Warning: Mixed model families detected ({model_family} vs {detected_family})")
            elif len(parts) == 2:
                # Legacy format: e.g., 'Transformers/Fully_Tuned'
                arch, var = parts
            else:
                print(f"Warning: Invalid model spec '{spec}' (expected 'Architecture/Variant' or 'Family/Architecture/Variant')")
                continue
    ...
    if model_family is None:
        # Auto-detect from available directories
        if Path('CAPTCHA_Models').exists():
            model_family = 'CAPTCHA_Models'
        elif Path('Bird_Models').exists():
            model_family = 'Bird_Models'
        else:
            raise ValueError("Could not detect model family. Please specify full path (e.g., CAPTCHA_Models/Transformers/Fully_Tuned)")

    # Load test classes
    print(f"\nUsing model family: {model_family}")
    print("Loading test data...")
    prep_import_path = f'{model_family}/Transformers/Fully_Tuned'
    sys.path.insert(0, prep_import_path)
    ...
    sys.path.remove(prep_import_path)

    # Validate checkpoints before running
    print("\nValidating checkpoints...")
    available_models = []
    for arch, var in models_to_run:
        # Prepend model_family to checkpoint path
        checkpoint_relative = MODELS_CONFIG[arch][var]['checkpoint']
        checkpoint_path = Path(model_family) / checkpoint_relative
    ...
    for idx, (architecture, variant) in enumerate(models_to_run, 1):
        ...
        try:
            df_model = analyze_model_margins(
                ...
                n_episodes, n_way, k_shot, q_query, device, seed, model_family
            )
...
Note: Evaluation settings (n_way, k_shot, etc.) are automatically loaded from the model's config.
        """
    )
    ...
    parser.add_argument(
        ...
        nargs='+',
        required=True,
        help='Models to analyze (e.g., CAPTCHA_Models/Transformers/Fully_Tuned)'
    )
...
def main():
    ...
    # Extract model family and architecture from first model to load config
    first_model = args.models[0]
    parts = first_model.split('/')
    if len(parts) == 3:
        model_family, architecture, variant = parts
    else:
        raise ValueError(f"Model path must be in format 'Family/Architecture/Variant', got: {first_model}")

    # Try to load config from model's few-shot results
    config_path = Path(model_family) / architecture / variant / 'outputs' / 'few_shot_results' / 'few_shot_results.json'
    ...
    if config_path.exists():
        ...
        with open(config_path, 'r') as f:
            ...
            n_way = config.get('n_way', n_way)
            k_shot_values = config.get('k_shot_values', [k_shot])
            # Use middle k_shot value if multiple available
            k_shot = k_shot_values[len(k_shot_values) // 2] if k_shot_values else k_shot
            q_query = config.get('q_query', q_query)
            n_episodes = config.get('n_episodes', n_episodes)
        print(f"Using model's config: {n_way}-way {k_shot}-shot, {q_query} queries, {n_episodes} episodes")
    else:
        print(f"Config file not found at {config_path}")
        print(f"Using defaults: {n_way}-way {k_shot}-shot, {q_query} queries, {n_episodes} episodes")
