...
class ResNet18_Pretrained(nn.Module):
    """
    Wrapper for torchvision's pretrained ResNet18 with ImageNet weights.
    Extracts 512-dim features from the layer before the final FC layer.
    All parameters are frozen for feature extraction only.
    """
    def __init__(self, flatten=True):
        super(ResNet18_Pretrained, self).__init__()
        import torchvision.models as models

        # Load pretrained ResNet18 (use legacy API for compatibility)
        ...
        print('Loaded ResNet18 with ImageNet pretrained weights (frozen)')

    def forward(self, x):
        """Forward pass through ResNet backbone (excluding FC layer)."""
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
