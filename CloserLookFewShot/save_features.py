...
#        modelfile   = get_resume_file(checkpoint_dir) #comment in 2019/08/03 updates as the validation of baseline/baseline++ is added
    ...
    # Load weights - skip checkpoint for pretrained models
    if params.model == 'ResNet18_Pretrained':
        # Pretrained model already has ImageNet weights loaded in __init__
        print('Using pretrained ResNet18 with ImageNet weights (frozen)')
    else:
        # Load trained checkpoint for non-pretrained models
