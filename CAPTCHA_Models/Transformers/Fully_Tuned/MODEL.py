...
class EmbeddingNetwork(nn.Module):
    """
    ViT_B_16 backbone with optional projection layer.
    Returns embedding_dim-D L2-normalized embeddings.
    """

    def __init__(self, pretrained: bool = True, embedding_dim: int = 768):
        super().__init__()
...
class PrototypicalNetwork(nn.Module):
    """
    Prototypical Network using EmbeddingNetwork (fixed ViT_B_16 backbone).
    """ 
...
def load_model(checkpoint_path: str, device: str = 'cuda', embedding_dim: int = 768) -> PrototypicalNetwork:
    checkpoint = torch.load(checkpoint_path, map_location=device, weights_only=False)

    embedding_net = EmbeddingNetwork(pretrained=False, embedding_dim=embedding_dim)
