...
class EmbeddingNetwork(nn.Module):
    """
    ResNet18 backbone with frozen layers and trainable final projection.
    Returns embedding_dim-D L2-normalized embeddings.
    """

    def __init__(self, pretrained: bool = True, embedding_dim: int = 256, freeze_backbone: bool = True):
        """
        Args:
        ...
        # Freeze all ResNet layers if requested
        if freeze_backbone:
            for param in self.encoder.parameters():
                param.requires_grad = False
            print("Frozen all ResNet18 backbone layers")

        # Add trainable projection layer: 512 -> embedding_dim
        self.projection = nn.Linear(512, embedding_dim)
        print(f"Added trainable projection layer: 512 -> {embedding_dim}")
...
class TrainingConfig:
    ...
    def __init__(self):
        ...
        # Save to script directory, not where command is run from
        import os
        script_dir = os.path.dirname(os.path.abspath(__file__))
        self.save_dir = os.path.join(script_dir, 'outputs', 'checkpoints')
...
def train_episode(model: PrototypicalNetwork, 
    ...
    model.train()
    model.embedding_network.encoder.eval() # To prevent adjustment of ResNet Batch Standardization
...
def load_model(checkpoint_path: str, device: str = 'cuda', embedding_dim: int = 256) -> PrototypicalNetwork:
    """
