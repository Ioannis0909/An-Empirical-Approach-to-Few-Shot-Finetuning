...
class FewShotConfig:
    ...
    def __init__(self):
        ...
        self.output_dir = os.path.join(script_dir, 'outputs', 'few_shot_results') 
...
def evaluate_k_shot(model: PrototypicalNetwork,
    ...
    # Episode generator with fixed seed for reproducible test episodes
    sampler = EpisodeSampler(
        ...
        seed=42,  # Fixed seed ensures identical test episodes across all models
    )
...
def evaluate_per_class(model: PrototypicalNetwork,
    ...
    sampler = EpisodeSampler(
        ...
        q_query=config.q_query,
        episodes_per_epoch=config.n_episodes,
        seed=42,  # Fixed seed for reproducible per-class evaluation
    )
